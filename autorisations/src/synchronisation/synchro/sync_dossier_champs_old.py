...
#                         f"id_nature={doc['id_nature']!r}, description={doc['description']!r} : {e}"
#                     )

#                     document_obj = None

#                     # ça ne devrait pas arriver mais au cas où on affecte le premier doc.
#                     # document_obj = (
#                     #     Document.objects
#                     #     .filter(emplacement=doc["emplacement"], titre__startswith=doc["titre"].rsplit(".", 1)[0], id_nature_id=doc["id_nature"], 
#                     #             description=doc["description"],)
#                     #     .order_by("id")
#                     #     .first()
#                     # )
...
#                         try:
#                             # DossierManifSportive existait avec l'ancien Numéro ?
#                             dossier_dm_ancien_num = DossierManifSportive.objects.filter(numero_dossier_declaration_manifestations=int(ancien_num)).first()

#                             # DossierManifSportive existe avec le nouveau Numéro ?
#                             dossier_dm_nouveau_num = DossierManifSportive.objects.filter(numero_dossier_declaration_manifestations=int(nouveau_num)).first()
...
#                             # Si DossierManifSportive existe avec le nouveau Numéro : On supprime la liaisonManifSportive de l'ancien numéro
#                             if dossier_dm_nouveau_num and liaisonManifSportive_ancien_num :
...
#                             # Si DossierManifSportive existe avec le nouveau Numéro
#                             if dossier_dm_nouveau_num :
#                                 logger.info(f"[Dossier {dossier.numero}] Numéro du dossier déclaration-manifestations changé : Un DossierManif existe avec le numéro {nouveau_num}")
                                
#                                 # Vérification si DossierManifestationLiaison existante pour notre DossierManifSportive (nouveau num)
#                                 liaison_existe_dossDM = DossierManifestationLiaison.objects.filter(id_dossier_manif=dossier_dm_nouveau_num).exists()

#                                 # Vérification si DossierManifestationLiaison existante pour notre DossierManifSportive (nouveau num) et Dossier
#                                 liaison_existe= DossierManifestationLiaison.objects.filter(id_dossier_manif=dossier_dm_nouveau_num, id_dossier=dossier).exists()
...
#                             # Si incohérence sur les liaisons
...
#                             elif liaison_existe :
#                                 logger.error(f"[Dossier {dossier.numero}] Numéro du dossier déclaration-manifestations changé ({nouveau_num}) : Un DossierManifestationLiaison existe deja entre les 2 dossiers")
                            
#                             # Création DossierManifestationLiaison du nouveau numéro
#                             elif dossier_dm_nouveau_num:
#                                 DossierManifestationLiaison.objects.create(id_dossier=dossier, id_dossier_manif=dossier_dm_nouveau_num)
