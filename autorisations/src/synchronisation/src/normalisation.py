...
def normalize_process(d):
    ...
    return {
        "demarche": demarche,
        "groupeinstructeurs_demarches": groupeinstructeurs_demarches,
        "champs": champs,
        "dossiers": dossiers,
...
def dossier_beneficiaire_normalize(d):
    ...
    for doss in d["dossiers"]["nodes"] :
        id_type_contact_externe = TypeContactExterne.objects.filter(type="demandeur_intermediaire").values_list("id", flat=True).first()
        ...
        elif doss['demandeur']['__typename'] == "PersonnePhysique" : # demandeur intermédiaire
            email_beneficaire = doss['demandeur']['email'] if doss['demandeur']['email'] else None
        ...
        for m in doss["messages"] :
            ...
            if m["attachments"]:
                for file in m["attachments"]:
                    ...
                    nom_fichier, extension_fichier = file["filename"]
...
if __name__ == "__main__":
    ...
    from synchronisation.src.functions import fetch_geojson, extraire_nom_et_extension, type_demande_from_nom_demarche, save_to_json
