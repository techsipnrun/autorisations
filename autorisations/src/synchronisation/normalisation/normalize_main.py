        ...
        if os.getenv("SYNCHRO_DM", "false") == "True" :
            ...
            if "Manifestations sportives" in d["title"] :
                ...
                try:
                    ...
                    # Appel API DM : On récupère les dossiers pour lesquels on a pas encore rendu d'avis
                    dic_dm, list_avis = call_api_dm.recup_avis_et_dossiers()

                    # On récupère les PJ des dossiers
                    # logger.info("Récupération des PJ sur Déclaration Manifestations...")
                    # dic_dm = call_api_dm.recup_pj_dossiers(dic_dm)
